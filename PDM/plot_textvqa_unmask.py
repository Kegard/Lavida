...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot bucketed token unmask-step ratios from TextVQA trace JSONL files."
    )
    parser.add_argument("--inputs", nargs="+", required=True, help="One or more TextVQA trace JSONL files.")
    parser.add_argument("--labels", nargs="+", default=None, help="Legend labels for each input file.")
    parser.add_argument("--field", default="token_unmask_steps", help="List-valued record field to plot.")
    parser.add_argument("--output", default="experiment/textvqa_unmask_plot.png")
    parser.add_argument("--title", default=None)
    parser.add_argument("--xlabel", default="Token Unmask Step")
    ...
    with open(path, "r", encoding="utf-8") as fin:
        for line in fin:
            ...
            if not isinstance(value, list):
                continue
            steps.extend(step for step in value if isinstance(step, int) and step > 0)
    return steps
...
def build_buckets(max_step, first_bucket_end, bucket_size):
    if max_step < 1:
        raise ValueError("No valid positive token unmask steps found to plot.")
